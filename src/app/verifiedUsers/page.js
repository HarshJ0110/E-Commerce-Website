...
export default function Verify() {
    ...
    const verifyUser = async (userId) => {
        try {
            const response = await axios.put("/api/users/nonVerifyUser", { id: userId, verify: false });
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get("/api/users/getVerifiedUsers");
    ...
    return (
        <div className="flex justify-center h-screen w-screen bg-white">
            <div className="m-4 sm:m-6 md:m-8 lg:m-10 xl:m-12 p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12 overflow-x-auto">
                ...
                <button className="mt-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"><Link href="/nonVerifiedUsers">Non-Verified User</Link></button>
                {users.length > 0 ? (
                            ...
                            {users.map((user, index) => (
                                <tr key={user._id} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
                                    ...
                                    <td className="py-2 px-4">
                                        <button onClick={() => verifyUser(user._id)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
